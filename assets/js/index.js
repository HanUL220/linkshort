...
function apiRequest() {
    ...
    if (customCode !== "") {
        fetch(`https://api.dok.do/link/create?url=${url}&code=${customCode}`, {
            method: "POST",
            headers: {
            ...
            if (data.response == 201) {
                document.getElementById("resultLinkSuccess").innerText = "dok.do/"+data.info.link_code;
                changeResult(1);
            }
        ...
        fetch(`https://api.dok.do/link/create?url=${url}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Access-Control-Allow-Origin": "*",
